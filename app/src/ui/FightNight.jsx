...
export default function FightNight({ fighter, done }) {
  ...
  const StatVs = ({ label, a, b }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
      <div style={{ flex: 1 }}><Bar v={a} color={T.neg} mirror /></div>
      <div style={{ width: 76, textAlign: "center", fontSize: 9, letterSpacing: 1.5, color: T.txt3, textTransform: "uppercase" }}>{label}<div style={{ color: T.txt, fontSize: 11, fontFamily: T.disp }}>{Math.round(a)} · {Math.round(b)}</div></div>
      <div style={{ flex: 1 }}><Bar v={b} color={T.steel} /></div>
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 60, overflowY: "auto", background: `radial-gradient(ellipse 90% 55% at 50% -5%, ${T.raised} 0%, ${C.bg} 60%)` }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: 14, position: "relative" }}>
        ...
        <div style={{ textAlign: "center", margin: "10px 0 14px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 4 }}>
            <div style={{ flex: 1, textAlign: "right" }}>
              <div style={{ fontFamily: T.disp, fontSize: 22, color: T.neg, lineHeight: 1, textTransform: "uppercase" }}>{fighter.name}</div>
              <div style={{ color: T.txt3, fontSize: 11 }}>{fighter.record.w}-{fighter.record.l} · {fighter.archetype}</div>
            </div>
            <div style={{ fontFamily: T.disp, fontSize: 26, color: T.gold, transform: "skewX(-10deg)", textShadow: "0 0 14px rgba(230,182,76,.5)" }}>VS</div>
            <div style={{ flex: 1, textAlign: "left" }}>
              <div style={{ fontFamily: T.disp, fontSize: 22, color: T.steel, lineHeight: 1, textTransform: "uppercase" }}>{opp.name}</div>
              <div style={{ color: T.txt3, fontSize: 11 }}>{(opp.record?.w ?? 0)}-{(opp.record?.l ?? 0)} · {opp.archetype}</div>
            </div>
          </div>
        </div>
        ...
        {state && stage !== "weighin" && (
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ flex: 1 }}><Bar v={hpA} color={hpA > 40 ? T.neg : "#ff2216"} h={14} skew mirror /></div>
              <div style={{ display: "flex", gap: 4 }}>
                {Array.from({ length: totalRounds }).map((_, i) => (
                  <div key={i} style={{ width: 8, height: 8, borderRadius: 4, background: i < state.scores.length ? (state.scores[i].a > state.scores[i].b ? T.neg : state.scores[i].b > state.scores[i].a ? T.steel : T.txt3) : i === state.scores.length && stage !== "result" ? T.gold : T.raised, boxShadow: i === state.scores.length && stage !== "result" ? `0 0 6px ${T.gold}` : "none" }} />
                ))}
              </div>
              <div style={{ flex: 1 }}><Bar v={hpB} color={T.steel} h={14} skew /></div>
            </div>
            <div style={{ display: "flex", gap: 6, marginTop: 4, alignItems: "center" }}>
              <div style={{ flex: 1 }}><Bar v={state.staA} color={T.pos} h={5} skew mirror /></div>
              <div style={{ width: 56, textAlign: "center", fontSize: 8, letterSpacing: 1.5, color: T.txt3 }}>HP / STA</div>
              <div style={{ flex: 1 }}><Bar v={state.staB} color={T.pos} h={5} skew /></div>
            </div>
            {/* Live Stats */}
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 9, color: T.txt3 }}>
              <span>👊 Strikes: <b style={{ color: T.neg }}>{totalLandA}</b></span>
              <span>🎯 Takedowns: <b style={{ color: T.gold }}>{totalTdA}</b></span>
              <span style={{ flex: 1, textAlign: "center" }}>|</span>
              <span>🎯 Takedowns: <b style={{ color: T.gold }}>{totalTdB}</b></span>
              <span>👊 Strikes: <b style={{ color: T.steel }}>{totalLandB}</b></span>
            </div>
          </div>
        )}
        ...
        {/* STAREDOWN */}
        {stage === "staredown" && (
          <Card accent={T.gold} style={{ textAlign: "center" }}>
            <H>🔥 Staredown · Press Conference</H>
            <div style={{ color: T.txt, fontSize: 14, marginBottom: 14 }}>
              Kedua fighter saling berhadapan. Pilih sikap yang akan <b style={{ color: T.gold }}>{fighter.name}</b> tunjukkan.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[
                { k: "Respectful", icon: "🤝", desc: "Footwork +5% · Pop +2 · Aman", color: T.pos },
                { k: "Professional", icon: "😐", desc: "Netral · Rep +2 jika menang", color: T.steel },
                { k: "Trash Talk", icon: "🗣️", desc: "Striking +8% (lawan +5%) · Pop +5 · Kalah = morale -8", color: T.neg },
              ].map((a) => (
                <div key={a.k} onClick={() => setAttitude(a.k)} style={{ border: `2px solid ${attitude === a.k ? a.color : T.line}`, background: attitude === a.k ? `${a.color}11` : "transparent", padding: 12, cursor: "pointer", ...{} }}>
                  <div style={{ fontSize: 28, marginBottom: 4 }}>{a.icon}</div>
                  <div style={{ fontFamily: T.disp, color: a.color, fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>{a.k}</div>
                  <div style={{ color: T.txt3, fontSize: 9, marginTop: 4, lineHeight: 1.3 }}>{a.desc}</div>
                </div>
              ))}
            </div>
            {attitude && <Btn wide color={T.gold} onClick={() => setStage("weighin")}>Lanjut ke Weigh-in</Btn>}
          </Card>
        )}
              ...
              <div style={{ textAlign: "center", marginTop: 12, fontSize: 12, color: cutInfo.pen ? T.neg : T.pos }}>
                ...
                {missedWeight && !weighinIssue && (
                  <div style={{ marginTop: 8, padding: 10, background: "rgba(225,75,68,.12)", border: "1px solid #e14b4466", ...{} }}>
                    <div style={{ color: T.neg, fontSize: 13, fontFamily: T.disp, letterSpacing: 1 }}>⚠️ MISS WEIGHT — {fighter.natWeight} lbs!</div>
                    <div style={{ color: T.txt3, fontSize: 11, margin: "6px 0" }}>{fighter.name} gagal mencapai {limit} lbs. Pilih tindakan:</div>
                    <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                      <Btn small color={T.neg} onClick={() => setWeighinIssue("catchweight")}>Catchweight (purse -30%)</Btn>
                      <Btn small color={T.txt3} onClick={() => setWeighinIssue("cancelled")}>Batalkan Fight</Btn>
                    </div>
                  </div>
                )}
                {weighinIssue === "catchweight" && <div style={{ color: T.neg, marginTop: 4 }}>Catchweight disetujui — purse dipotong 30%.</div>}
                {weighinIssue === "cancelled" && <div style={{ color: T.neg, marginTop: 4 }}>Fight dibatalkan — rep & relasi turun.</div>}
              </div>
            </Card>
            <Card>
              <H>Game Plan</H>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {Object.entries(GAME_PLANS).map(([k, d]) => (
                  <div key={k} onClick={() => setPlan(k)} style={{ border: `1px solid ${plan === k ? T.gold : T.line}`, background: plan === k ? "rgba(230,182,76,.08)" : "transparent", padding: 10, cursor: "pointer", ...{} }}>
                    <div style={{ fontFamily: T.disp, color: plan === k ? T.gold : T.txt, fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>{k}</div>
                    <div style={{ color: T.txt3, fontSize: 10, marginTop: 3 }}>{d}</div>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 10, alignItems: "center" }}>
                <span style={{ fontSize: 10, color: T.txt3, textTransform: "uppercase", letterSpacing: 1 }}>View:</span>
                {[{ k: "tick", label: "Tick-by-Tick" }, { k: "summary", label: "Round Summary" }, { k: "skip", label: "Skip to Result" }].map((m) => (
                  <button key={m.k} onClick={() => setViewMode(m.k)} style={{ background: viewMode === m.k ? T.gold : T.raised, color: viewMode === m.k ? "#0a0d14" : T.txt3, border: `1px solid ${T.line}`, padding: "3px 8px", fontSize: 10, cursor: "pointer", fontFamily: T.disp, letterSpacing: 1, ...{} }}>{m.label}</button>
                ))}
              </div>
              <div style={{ marginTop: 12 }}><Btn wide color={T.neg} onClick={startFight}>🔔 Bunyikan Bel</Btn></div>
            </Card>
          </>
        )}
        ...
        {/* FIGHTER ENTRANCE */}
        {stage === "entrance" && (
          <Card accent={T.gold} style={{ textAlign: "center", padding: 30 }}>
            <div style={{ fontSize: 48, marginBottom: 4, animation: "koflash .8s ease both" }}>🚶</div>
            <H color={T.gold}>Fighter Entrance</H>
            <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 14 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: T.disp, color: T.neg, fontSize: 20, textTransform: "uppercase" }}>{fighter.name}</div>
                <div style={{ color: T.txt3, fontSize: 12, marginTop: 2 }}>{fighter.record.w}-{fighter.record.l} · {fighter.archetype}</div>
                <div style={{ color: T.txt3, fontSize: 10, marginTop: 1 }}>{fighter.weightClass}</div>
              </div>
              <div style={{ fontFamily: T.disp, fontSize: 26, color: T.gold, alignSelf: "center" }}>VS</div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: T.disp, color: T.steel, fontSize: 20, textTransform: "uppercase" }}>{opp.name}</div>
                <div style={{ color: T.txt3, fontSize: 12, marginTop: 2 }}>{(opp.record?.w ?? 0)}-{(opp.record?.l ?? 0)} · {opp.archetype}</div>
                <div style={{ color: T.txt3, fontSize: 10, marginTop: 1 }}>{opp.weightClass || ""}</div>
              </div>
            </div>
            <div style={{ color: T.txt3, fontSize: 11, marginTop: 16, animation: "goldglow 1.5s infinite" }}>
        ...
        {(stage === "round" || stage === "corner" || stage === "skipSummary") && roundLog && (() => {
          ...
          return (
          <Card accent={T.line}>
            <H color={T.txt}>Round {rnd} · Commentary</H>
            {displayLog.map((l, i) => {
              const showLine = viewMode !== "tick" ? true : i <= tickIdx;
              if (!showLine) return null;
              return (
              <div key={i} className="rise" style={{ animationDelay: `${i * 0.12}s`, color: l.includes("KO") || l.includes("SUB") ? T.gold : T.txt, fontSize: 13, marginBottom: 6, paddingLeft: 10, borderLeft: `2px solid ${l.includes("KO") || l.includes("SUB") ? T.gold : T.line}` }}>{l}</div>
              );
            })}
            {viewMode === "tick" && stage === "round" && tickIdx < displayLog.length && (
              <div className="rise" style={{ color: T.txt3, fontSize: 10, marginTop: 4 }}>▌ Play-by-play · tick {tickIdx + 1}/{displayLog.length}</div>
            )}
          </Card>)})()}
        {stage === "skipSummary" && (
        ...
        {stage === "corner" && !docCheck && (
          <Card accent={T.gold} style={{ boxShadow: "0 0 30px rgba(230,182,76,.12)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontFamily: T.disp, color: T.gold, fontSize: 15, letterSpacing: 2 }}>🪑 CORNER — 60 DETIK</div>
              <div style={{ fontFamily: T.disp, fontSize: 22, color: timer <= 5 ? T.neg : T.txt, minWidth: 40, textAlign: "right", animation: timer <= 5 ? "goldglow 1s infinite" : "none" }}>0:{String(Math.max(timer, 0)).padStart(2, "0")}</div>
            </div>
            <div style={{ background: "#0a0e17", padding: 10, marginBottom: 10, ...{} }}>
              <div style={{ display: "flex", gap: 12, fontSize: 10, marginBottom: 6 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ color: T.txt3, textTransform: "uppercase", fontSize: 8, marginBottom: 2 }}>{fighter.name}</div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <Bar v={state?.staA || 100} color={state?.staA > 50 ? T.pos : state?.staA > 25 ? T.gold : T.neg} h={6} />
                    <span style={{ color: T.txt, fontSize: 11, fontFamily: T.disp, width: 30 }}>{Math.round(state?.staA || 100)}%</span>
                  </div>
                  <div style={{ color: T.txt3, fontSize: 8, marginTop: 1 }}>HP: {Math.round(100 - (state?.dmgA || 0))}% {state?.bodyDmgA > 20 ? "· Body hurt" : ""} {state?.legDmgA > 15 ? "· Legs hurt" : ""}</div>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: T.txt3, textTransform: "uppercase", fontSize: 8, marginBottom: 2 }}>{opp.name}</div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <Bar v={state?.staB || 100} color={state?.staB > 50 ? T.pos : state?.staB > 25 ? T.gold : T.neg} h={6} />
                    <span style={{ color: T.txt, fontSize: 11, fontFamily: T.disp, width: 30 }}>{Math.round(state?.staB || 100)}%</span>
                  </div>
                  <div style={{ color: T.txt3, fontSize: 8, marginTop: 1 }}>HP: {Math.round(100 - (state?.dmgB || 0))}% {state?.bodyDmgB > 20 ? "· Body hurt" : ""} {state?.legDmgB > 15 ? "· Legs hurt" : ""}</div>
                </div>
              </div>
              <div style={{ display: "flex", gap: 3, alignItems: "center", marginBottom: 4 }}>
                <span style={{ fontSize: 8, color: T.txt3, textTransform: "uppercase", marginRight: 4 }}>Score:</span>
                {state?.scores?.map((s, i) => (<span key={i} style={{ fontSize: 9, color: s.a > s.b ? T.neg : s.b > s.a ? T.steel : T.txt3, fontFamily: T.disp }}>{s.a > s.b ? "A" : s.b > s.a ? "B" : "D"}</span>))}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 4 }}>
                <span style={{ fontSize: 8, color: T.txt3, textTransform: "uppercase" }}>Momentum:</span>
                <div style={{ flex: 1, height: 4, background: "#141a28", position: "relative" }}>
                  <div style={{ position: "absolute", top: 0, bottom: 0, left: "50%", width: `${Math.abs(state?.mom || 0) * 0.5}%`, background: (state?.mom || 0) > 0 ? T.neg : T.steel, transform: (state?.mom || 0) > 0 ? undefined : "translateX(-104%)" }} />
                  <div style={{ position: "absolute", top: "50%", left: "50%", width: 2, height: 4, background: T.txt3, transform: "translate(-50%, -50%)" }} />
                </div>
                <span style={{ fontSize: 8, color: (state?.mom || 0) >= 0 ? T.neg : T.steel, fontFamily: T.disp, width: 16, textAlign: "center" }}>{(state?.mom || 0) >= 0 ? `${Math.round(state?.mom || 0)}%` : `${Math.round(Math.abs(state?.mom || 0))}%`}</span>
              </div>
              <div style={{ fontSize: 10, color: T.gold, fontStyle: "italic" }}>
            ...
            {cornerOpts.map((o) => (
              <div key={o.k} onClick={() => setCorner(o.k)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", border: `1px solid ${corner === o.k ? T.gold : T.line}`, background: corner === o.k ? "rgba(230,182,76,.08)" : "transparent", padding: "8px 10px", marginBottom: 6, cursor: "pointer", ...{} }}>
                <div>
                  <span style={{ fontFamily: T.disp, color: corner === o.k ? T.gold : T.txt, fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>{o.label}</span>
                  <div style={{ fontSize: 8, color: corner === o.k ? T.gold : T.txt3 }}>{o.trade}</div>
                </div>
                <span style={{ color: T.txt3, fontSize: 10 }}>{o.desc}</span>
              </div>
            ))}
            <Btn wide color={T.neg} onClick={nextRound}>🔔 Round {rnd + 1}</Btn>
          </Card>
        )}

        {/* KNOCKDOWN */}
        {stage === "knockdown" && roundLog?.knockdown && (
          <Card accent={T.neg} style={{ textAlign: "center", boxShadow: "0 0 40px rgba(225,75,68,.25)" }}>
            <div style={{ fontSize: 56, marginBottom: 4, animation: "koflash .8s ease both" }}>💥</div>
            <H color={T.neg}>DOWN! {roundLog.knockdown.name} TERJATUH!</H>
            <div style={{ color: T.txt, fontSize: 14, marginBottom: 12 }}>
              Wasit langsung menghentikan pertarungan!
            </div>
            <Btn color={T.neg} onClick={() => {
        ...
        {/* DOCTOR STOPPAGE */}
        {stage === "corner" && docCheck && (
          <Card accent={T.neg} style={{ textAlign: "center", boxShadow: "0 0 30px rgba(225,75,68,.18)" }}>
            <H color={T.neg}>🚑 Doctor Check — Cuts & Injury</H>
            <div style={{ color: T.txt, fontSize: 13, marginBottom: 8 }}>Dokter memasuki ring untuk memeriksa kondisi fighter.</div>
            <div style={{ display: "flex", justifyContent: "center", gap: 20, marginBottom: 14, fontSize: 11 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: T.disp, color: T.neg, fontSize: 14 }}>{fighter.name}</div>
                <div style={{ color: cutA >= 6 ? T.neg : cutA >= 4 ? T.gold : T.txt3 }}>🩸 Cut level: {cutA}/10</div>
              </div>
              <div style={{ color: T.txt3, alignSelf: "center" }}>VS</div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: T.disp, color: T.steel, fontSize: 14 }}>{opp.name}</div>
                <div style={{ color: cutB >= 6 ? T.neg : cutB >= 4 ? T.gold : T.txt3 }}>🩸 Cut level: {cutB}/10</div>
              </div>
            </div>
            <div style={{ color: T.txt3, fontSize: 10, marginBottom: 12 }}>Cut tinggi = risiko cedera permanen. Dokter bisa menghentikan fight.</div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
              <Btn small color={T.pos} onClick={() => { setDocCheck(false); nextRound(); }}>Lanjutkan Fight</Btn>
              <Btn small color={T.neg} onClick={doctorRetire}>Hentikan (TKO Loss)</Btn>
            </div>
          </Card>
        )}
        ...
        {stage === "result" && result && (
          <div style={{ position: "relative" }}>
            ...
            <Card accent={result.won ? T.gold : T.neg} style={{ textAlign: "center", paddingTop: 26, paddingBottom: 22 }}>
              <div style={{ fontFamily: T.disp, fontSize: 46, letterSpacing: 4, color: result.draw ? T.txt3 : result.won ? T.gold : T.neg, display: "inline-block", padding: "2px 18px", border: `3px solid ${result.draw ? T.txt3 : result.won ? T.gold : T.neg}`, animation: "belldrop .6s cubic-bezier(.2,1.4,.4,1) both", textTransform: "uppercase", ...{} }}>
                {result.draw ? "Draw" : result.won ? "Victory" : "Defeat"}
              </div>
              <div style={{ color: T.txt, fontSize: 14, margin: "14px 0 4px" }}>
                {result.draw ? <b style={{ color: T.txt3 }}>{result.how}</b> : <b style={{ color: result.won ? T.neg : T.steel }}>{result.won ? fighter.name : opp.name}</b>}{result.draw ? " — " : " menang via "}{result.how} · Round {result.r}
              </div>
              {result.won && fighter.booked.title && <div style={{ fontFamily: T.disp, color: T.gold, fontSize: 18, letterSpacing: 2, animation: "goldglow 2s infinite" }}>👑 AND {fighter.booked.defense ? "STILL" : "NEW"} {fighter.booked.titleTier ? fighter.booked.titleTier.toUpperCase() : ""} CHAMPION</div>}
              <div style={{ color: T.txt3, fontSize: 12, margin: "8px 0 14px" }}>
                Purse {fmt$(fighter.booked.show + (result.won ? fighter.booked.winBonus : 0))} → camp cut {Math.round(((fighter.contract && fighter.contract.managerCut) || 0.18) * 100)}% = <b style={{ color: T.pos }}>{fmt$((fighter.booked.show + (result.won ? fighter.booked.winBonus : 0)) * ((fighter.contract && fighter.contract.managerCut) || 0.18))}</b>
              </div>
              ...
              }} style={{ marginLeft: 8, background: T.raised, color: T.txt3, border: `1px solid ${T.line}`, padding: '8px 14px', fontSize: 10, cursor: 'pointer', fontFamily: 'monospace', letterSpacing: 1, borderRadius: 6 }}>📋 Copy Fight Report</button>
            </Card>
          </div>
        )}
      </div>
