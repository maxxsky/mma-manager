...
import { ARCH_COLOR, TRAITS, TRAINING } from "../engine/data.js";
import { avgSkill } from "../engine/fighter.js";
import { rankOf } from "../engine/rankings.js";
import { T, Panel, Eyebrow, Tag, Ovr, Mono, heat, Icon, ICONS } from "./theme.jsx";
import FighterDetail from "./FighterDetail.jsx";

const ATTR_KEYS = ["striking", "wrestling", "bjj", "footwork", "strength", "cardio", "chin", "fightIQ"];
const ATTR_COLS = ["STR", "WRE", "BJJ", "FTW", "PWR", "CAR", "CHN", "IQ"];
...
export default function Roster({ g, setTab, up }) {
  ...
  if (detailFighter) {
    ...
    return f ? <FighterDetail f={f} g={g} onBack={() => setDetailFighter(null)} up={up} /> : null;
  }
  ...
  return (
    <Panel pad={0} style={{ overflow: "hidden" }}>
      ...
      <div style={{ display: "grid",
        ...
        {["Fighter", "OVR", ...ATTR_COLS, "Status", "Cond"].map((c, i) => (
      ...
      {g.roster.map((f) => {
        ...
        return (
          <div key={f.id} className="row"
            ...
            {/* Attribute columns — heat-colored */}
            {ATTR_KEYS.map((k) => (
            ...
            {/* Condition: morale + overtraining mini-bars */}
