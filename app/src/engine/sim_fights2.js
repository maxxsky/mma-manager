...
const ARCHETYPES = {
  Boxer: { striking: 1.20, footwork: 1.15, wrestling: 0.8, bjj: 0.7 },
  "Muay Thai": { striking: 1.20, strength: 1.10, bjj: 0.7, wrestling: 0.75 },
  Wrestler: { wrestling: 1.40, strength: 1.15, striking: 0.75, bjj: 0.95 },
  "BJJ Specialist": { bjj: 1.45, wrestling: 1.10, striking: 0.7, footwork: 0.85 },
...
function simRound(rnd, A, B, stA, stB, planA, cornerA, cutPenA, momentum) {
  ...
  for (let ex = 0; ex < nEx; ex++) {
    ...
    if (exType === "strike" || exType === "power") {
      ...
      const p = clamp(0.32 + (effAttr(A, "wrestling", stA, {}) - effAttr(B, "wrestling", stB, {})) / 80, 0.08, 0.90);
      if (random() < p) {
  ...
  const drainA = R(10, 16) * agg * (65 / clamp(A.attrs.cardio, 30, 95));
