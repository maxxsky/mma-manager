...
import { clamp, pick, R, RI, random } from "./rng.js";
import { REGIONS, ARCHETYPES } from "./data/archetypes.js";
...
/** Pick a random region from REGIONS — shared across fighter factories */
export function pickRegion() {
  return pick(Object.keys(REGIONS));
}

/** Pick archetype with regional bias (~55% region's native arch) */
export function pickArchetypeForRegion(region) {
  const rd = REGIONS[region];
  return random() < 0.55 ? rd.arch : pick(Object.keys(ARCHETYPES));
}
