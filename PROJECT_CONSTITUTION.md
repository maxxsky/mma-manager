...
### 3.1 Maintainability over short-term speed
Code will be read and modified many times. Favor solutions that are easy to understand and safe to change. The fastest fix today is worthless if it costs hours of confusion tomorrow.

### 3.2 Technical debt must be intentional
Every shortcut carries a cost. If you take a shortcut, document the trade-off and add a tracking issue. Technical debt that nobody knows about is no longer debt — it's a trap.

### 3.3 Simplicity over generality
Solve the problem at hand, not every possible future problem. Generic abstractions for speculative use cases add complexity without value. Add generality only when the need is proven.

### 3.4 Data First
Business rules belong in the engine (`src/engine/`). The UI reflects engine state — it is not the source of truth. Game state must never be derived from what the UI renders, and no gameplay logic should depend on a component's lifecycle.

---

## 4 — Core Design Principles

### 4.1 Emergence over scripted content
...
### 4.2 Simulation depth over presentation polish
...
### 4.3 Strategic depth over complexity
...
### 4.5 Long-term saves are sacred
...
### 5.2 Orchestration over monolithic functions
...
### 5.3 Registry over switch/if-else chains
...
### 5.5 Deterministic simulation where possible
...
### 6.1 Clean code over clever code
...
### 6.4 Test the happy path, guard the edge cases
...
### 6.6 Self-documenting code over comments
...
6. **Architecture preserved** — the implementation follows existing patterns and does not add unnecessary abstractions.
7. **Pushed** — the change is committed and pushed to the repository.
8. **Verified** — the deployed version works on the VPS (not just local dev server).
...
When uncertain about the correct approach:

- Choose the solution with the **smallest possible change**.
- Follow **patterns that already exist** in the codebase.
- **Preserve compatibility** — do not break what already works.
- **Do not assume** — verify before deciding.

---

## 13 — Existing Pattern First

Before introducing a new implementation pattern:

1. Search the codebase for similar problems and their solutions.
2. Study multiple existing implementations to understand the established conventions.
3. Follow existing conventions whenever possible.
4. Introduce a new pattern only if existing ones cannot reasonably solve the problem — and explain why.
